...
{
	/* this was ported from later LK version
	 * after lk 4.17
	 * flow_type FLOW_RSS mask should be set to zero
	 */
	switch (flow_type & ~(FLOW_EXT | FLOW_MAC_EXT)) {
	case TCP_V4_FLOW:
				...
				const struct mvpp2_cls_flow *flow)
{
	...
	mvpp2_cls_flow_lkp_type_set(&fe, MVPP22_FLOW_ETHERNET);
...
static void mvpp2_port_c2_cls_init(struct mvpp2_port *port)
{
	...
	/* Set lkp_type */
	c2.tcam[4] |= MVPP22_CLS_C2_LKP_TYPE(MVPP22_FLOW_ETHERNET);
