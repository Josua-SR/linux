...
static int otx2_xdp_xmit(struct net_device *netdev, struct xdp_buff *xdp)
{
	struct otx2_nic *pf = netdev_priv(netdev);
	...
	/* Abort xmit if xdp queue is not configured */
	if (unlikely(!sq))
		return -ENXIO;

	len = xdp->data_end - xdp->data;
	dma_addr = otx2_dma_map_page(pf, virt_to_page(xdp->data),
				     offset_in_page(xdp->data), len,
				     DMA_TO_DEVICE, DMA_ATTR_SKIP_CPU_SYNC);
	if (dma_mapping_error(pf->dev, dma_addr))
		return -ENOMEM;

	err = otx2_xdp_sq_append_pkt(pf, dma_addr, len, qidx);
	if (!err) {
		otx2_dma_unmap_page(pf, dma_addr, len, DMA_TO_DEVICE,
...
static int otx2_xdp(struct net_device *netdev, struct netdev_xdp *xdp)
{
	struct otx2_nic *pf = netdev_priv(netdev);
